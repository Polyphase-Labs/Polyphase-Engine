...
struct LocalBuildState
{
    ...
    /**
     * @brief Obfuscate shipped .oct / .lua / AssetRegistry.txt in the package.
     *
     * Independent of mEmbedded -- a Static build can also be embedded, in which
     * case the generated byte arrays are obfuscated as well.
     */
...
public:
    ...
    /**
     * @brief Static Build: obfuscate shipped content under the staging dir.
     *
     * Rewrites every .oct, every .lua and AssetRegistry.txt in place. Idempotent
     * -- already-wrapped files are skipped, so a re-run is safe. Everything else
     * (the executable, Config.ini, the .octp, shaders, icons, raw assets) is left
     * readable on purpose.
     */
     ...
     */
    bool PackPackagedContent(const std::string& packagedDir,
