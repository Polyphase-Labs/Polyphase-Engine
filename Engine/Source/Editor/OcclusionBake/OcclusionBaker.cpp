...
void GatherOccluderTriangles(Primitive3D* prim, std::vector<BvhTri>& outTris)
{
    StaticMesh3D* meshNode = prim->As<StaticMesh3D>();
    if (meshNode != nullptr && !MaterialCanOcclude(meshNode->GetMaterial()))
        return;

    GatherPrimitiveTriangles(prim, outTris);
}
