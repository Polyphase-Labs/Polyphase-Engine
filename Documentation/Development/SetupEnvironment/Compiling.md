...
1. Follow [macOS setup](Mac.md) once (Command Line Tools, Vulkan SDK, the devkitPro console toolchains, `bash Tools/prebuild_mac.sh`).
2. From the project's root directory `cd Standalone`
3. Run `make -f Makefile_Mac_Editor -j$(sysctl -n hw.ncpu)` (add `MAC_ARCH=universal` for an Apple Silicon + Intel binary; the default is this Mac's architecture)
...
3. Packaging for macOS (the `macOS (App Bundle)` target) requires a macOS host with the Xcode Command Line Tools and the Vulkan SDK; see [Platforms/Mac/Packaging.md](../Platforms/Mac/Packaging.md). It cannot be built through Docker. Wii, GameCube and 3DS packaging on a Mac uses the native devkitPro toolchains from [macOS setup](Mac.md#install-devkitpro-toolchains).
