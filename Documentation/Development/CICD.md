...
| `PolyphaseEditor-<version>-macos-universal.dmg` | `build-mac` → `Installers/build_dmg_mac.sh` (`MAC_ARCH=universal`, `macos-15` runner) | GitHub Release asset. Signed (ad-hoc by default) universal `Polyphase.app` disk image for Apple Silicon and Intel Macs; see `Documentation/Development/Platforms/Mac/Overview.md`. |
