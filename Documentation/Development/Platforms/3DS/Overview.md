...
| Rotate / Speed / Rotation Min / Max | on / 30 deg/s / 0 / 360 | Rotation about the vertical axis. Min 0 to Max 360 = continuous spin; a smaller span sways between the two angles with eased reversals; Rotate off = static at Min. Keys are placed on whole frames at least 15 frames apart and a cycle is never shorter than 1 s (the HOME Menu freezes on denser keys), so very fast short sways get fewer samples; a wider range at a lower speed looks smoother |
...
**3D banners** take a Scene asset: every visible `StaticMesh3D` is exported as glTF (world transform, first material texture, colour, blend mode, two-sidedness), auto-fitted into the HOME Menu camera and optionally rotated, then converted with pycgfx and wrapped by bannertool. Lighting comes from the scene too: the first visible `DirectionalLight3D` (direction, colour times intensity) becomes the banner's light, and the scene's Ambient Light Colour becomes every material's ambient term. Without a directional light the model gets pycgfx's default white headlight from the camera. The conversion runs through `Standalone/3DS/banner_cgfx.py`, which calls pycgfx as a library and then applies the lighting, so the pycgfx download is pinned to a fixed commit. Skeletal meshes, particles and text are skipped. Keep textures small; the converter refuses models over 512 KB and the editor falls back to the image banner with a warning. pycgfx has **no license file**, so the engine never bundles it: **Install pycgfx** in Preferences downloads the main branch on your explicit request and runs `pip install --user gltflib pillow`; you can also `git clone https://github.com/skyfloogle/pycgfx` and set the folder in Preferences.
