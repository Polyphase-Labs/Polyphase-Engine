    ...
    private LuaExpressionSyntax VisitPropertyOrEventIdentifierName(IdentifierNameSyntax node, ISymbol symbol, bool isProperty, out bool isField) {
      ...
      if (isProperty) {
        ...
        if (IsPropertyTemplate(propertySymbol)) {
          ...
          var propertyTemplate = new LuaPropertyTemplateExpressionSyntax(get, set);
          // POLYPHASE PATCH: implicit-this access (bare `Prop` inside the class) never
          // reaches BuildFieldOrPropertyMemberAccessExpression, leaving GetExpression
          // null and crashing the renderer. Populate the receiver eagerly; an explicit
          // member access will Update() again and overwrite, which is harmless.
          if (!propertySymbol.IsStatic) {
            var thisReceiver = LuaIdentifierNameSyntax.This;
            var getExpression = get != null
              ? (LuaCodeTemplateExpressionSyntax)BuildCodeTemplateExpression(get, new LuaSymbolNameSyntax(thisReceiver))
              : null;
            propertyTemplate.Update(thisReceiver, getExpression);
          }
